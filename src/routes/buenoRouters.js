...
// Cria um modelo específico para a tabela 'bueno'
const modelBueno = createModel('bueno');
...
router.get('/', async (req, res) => {
    try {
        const records = await modelBueno.getAll();
        res.json(records);
    } catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        const record = await modelBueno.getById(req.params.id);
        if (record) {
            res.json(record);
        } else {
...
router.post('/', async (req, res) => {
    try {
        ...
        const newRecord = await modelBueno.create(pesquisa1, pesquisa2, pesquisa3, comentario);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updatedRecord = await modelBueno.update(req.params.id, pesquisa1, pesquisa2, pesquisa3, comentario);
        if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
        await modelBueno.remove(req.params.id);
        res.status(204).end();
    } catch (err) {
