...
// Cria um modelo específico para a tabela 'belavista'
const modelBelaVista = createModel('belavista');
...
router.get('/', async (req, res) => {
    try {
        const records = await modelBelaVista.getAll();
        res.json(records);
    } catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        const record = await modelBelaVista.getById(req.params.id);
        if (record) {
            res.json(record);
        } else {
...
router.post('/', async (req, res) => {
    try {
        ...
        const newRecord = await modelBelaVista.create(pesquisa1, pesquisa2, pesquisa3, comentario);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updatedRecord = await modelBelaVista.update(req.params.id, pesquisa1, pesquisa2, pesquisa3, comentario);
        if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
        await modelBelaVista.remove(req.params.id);
        res.status(204).end();
    } catch (err) {
